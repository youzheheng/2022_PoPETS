...
# Youzhe Heng
#
# Contact: y.heng@uni-mannheim.de
#
# University of Mannheim
# =============================================================================

import hashlib  # A standard Python library
from random import randint, seed  # For random hashing
...
# BCH
#code_length = 511
#code_minimum_distance = 223
#bch=codes.BCHCode(GF(2), code_length, code_minimum_distance)
#H=bch.parity_check_matrix()
#code_dim = 28


# Golay
#code_length_golay = 24
#golay=codes.GolayCode(GF(2))
#H_golay = golay.parity_check_matrix()
#code_dim_golay = 12
...
class DoubleHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      encoded_q_gram = q_gram.encode()
      hex_str1 = self.hash_funct1(encoded_q_gram).hexdigest()
      int1 =     int(hex_str1, 16)

      hex_str2 = self.hash_funct2(encoded_q_gram).hexdigest()
    ...
    #bf = bitarray.bitarray(H*vector(GF(2),bf))  
    #if get_q_gram_pos == True:
      #for i in range(len(bf)):
        #if bf[i] == 1:
          #q_gram_pos_set.add(i)
    #if get_q_gram_pos == True:
      #q_gram_pos_dict[q_gram] = q_gram_pos_set
...
class TripleHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      hex_str3 = self.hash_funct3(encoded_q_gram).hexdigest()
...
class RandomHashing():
  ...
  def hash_q_gram_set(self, q_gram_set, salt_str=None):
    ...
    for q_gram in q_gram_set:
      ...
      #
      encoded_q_gram = q_gram.encode()
      hex_str = self.hash_funct(encoded_q_gram).hexdigest()
      random_seed = seed(int(hex_str, 16))

      for i in range(k):
        pos_i = randint(0, bf_len_m1)
        bf[pos_i] = 1
